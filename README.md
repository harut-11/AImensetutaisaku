...
1.  `感情アーカイブ.bat` ファイルを以下のように書き換えてください。
    ...
    ```batch
    rem 
    cd /d "C:\AImensetutaisaku" ←　ここをあなたのプロジェクトのフォルダパスに変更してください
    ```

4.  `"C:\AImensetutaisaku"` の部分を、あなたがこのプロジェクトを保存した**実際のフォルダパス**に書き換えてください。
5.  上書き保存してファイルを閉じます。

設定後は、`面接対策アプリ.bat` を**ダブルクリックするだけ**でサーバーが起動し、コマンドプロンプト（黒い画面）が開きます。
.batはどこでも配置可能です。
---
