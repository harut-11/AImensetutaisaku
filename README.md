...
# 開発理由
 就職活動中、面接練習をしたいけれど、こんな悩みありませんか？
- 学生時代に就職活動で面接対策をしたいが、一人で学校のキャリアセンターに行くのが不安
- 面接対策をしてほしいが、就活アドバイザーやキャリアセンターが営業時間外だや予約が取れなかったりして、練習できないことがある。
- 自分のペースで、もっと気軽に面接対策をしたい
> **このアプリは、そんな学生たちの声に応えるために開発しました。  
いつでも、どこでも、あなたに寄り添った面接対策をサポートします。**
---
...
- 推奨版( Windows Installer(.msl) )をダウンロードしてください（本プロジェクトでは v20.15.0を推奨）
...
プロジェクトには `run_server.bat` (※ファイル名は適宜読み替えてください) のようなバッチファイルが含まれています。これは `node server.js` コマンドを簡単に実行するためのものです。

**ただし、使用前に設定が必要です。**

1.  `run_server.bat` ファイルを右クリックし、「**編集**」を選択します。
2.  メモ帳などでファイルが開かれます。
3.  以下の行を見つけてください。
...
4.  `"C:\AImensetutaisaku"` の部分を、あなたがこのプロジェクトを保存した**実際のフォルダパス**（`server.js` がある場所）に書き換えてください。
5.  上書き保存してファイルを閉じます。

設定後は、`run_server.bat` を**ダブルクリックするだけ**でサーバーが起動し、コマンドプロンプト（黒い画面）が開きます。
# デスクトップに置くことも可能です。
---
...
- **SheetJS（xlsx-style / sheetjs-style）**  
  面接で聞かれる質問はある程度パターンが決まっているため、質問集をExcelで管理しています。  
これにより、業界や職種ごとに質問を簡単に追加・編集でき、基礎的な質問から応用的な質問まで幅広く対応することが可能です。
...
## 今後の展望
- Gemini有料プランを活用し、質問をAIに生成させる機能を追加
